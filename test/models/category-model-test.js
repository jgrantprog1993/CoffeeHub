...
suite("Location Model tests", () => {
  ...
  setup(async () => {
    db.init("json");
    await db.locationStore.deleteAllLocation();
  ...
  test("delete all locations", async () => {
    let returnedLocation = await db.locationStore.getAllLocation();
    assertSubset(returnedLocation.length, 3);
    await db.locationStore.deleteAllLocation();
    returnedLocation = await db.locationStore.getAllLocation();
  ...
  test("delete One Location - success", async () => {
    ...
    const returnedLocation = await db.locationStore.getAllLocation();
  ...
  test("delete One Location - fail", async () => {
    ...
    const allLocation = await db.locationStore.getAllLocation();
